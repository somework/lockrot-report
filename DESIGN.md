...
| PD-GLOSSARY-4/PD-GLOSSARY-5 | a verdict pill linked out, unreachable offline; a row's pill had no definition, then trapped a click meant for the row | the detail header's pill opens its definition in a popover; a row's own pill stays plain, its title carrying the definition, so the row's click opens the package |
