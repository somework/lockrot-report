...
src/
  ...
  domain/                 pure functions, no DOM, no Preact; unit-tested
    ...
    advisoryLedger.ts     the Advisories tab's counts, reported-ago axis, a row chip's title (PD-ADV-*)
    ...
    share.ts              print and share: scope rollup, copied summary, running line (PD-PRINT-*)
  state/
...
fixtures/bundles/         real payloads, and hand-built ones (mini*.json) for shapes no real one carries
...
| PD-SUMMARY-9                | (new) the flagged packages' scope split lived only in the rail, folded away on a phone                                                                                                                                         | one line under the summary figure, from `Finding.dev` and `Finding.direct`: "51 in production, 18 dev-only · 20 required directly, 49 pulled in"                                                                                                                                                                                                                                                                                                   |
| PD-PRINT-1                  | print was the open tab only, with no control for it; a print from Run data had no summary                                                                                                                                      | a "Print / PDF" header control; every print is one document in numbered sections (summary, Findings, Advisories in full, Blast radius's ranked rows, Run data; All packages only when printed from that tab, and it says so), mounted on `beforeprint` or a print medium, never on screen; filters and search do not apply                                                                                                                         |
| PD-PRINT-2                  | past page 1 a printed page did not say what it was; the dark theme printed pale ink on white; the packages table was clipped at the margin                                                                                     | an `@page` running line (project, data date, lockrot) and "page N of M" where page-margin boxes exist; paper always uses the light tokens; the table's cells wrap                                                                                                                                                                                                                                                                                  |
| PD-PRINT-3                  | (new)                                                                                                                                                                                                                          | "Copy summary": the answer as three lines of plain text on the clipboard, or selected in a popover when the clipboard refuses                                                                                                                                                                                                                                                                                                                      |
| PD-PRINT-4                  | past its first page a printed Findings row's bar had no scale and no group over it; All packages' repeated head printed blank; Findings always opened a fresh page, and a section's last line could drag the next one overleaf | on paper each Findings priority group is a table whose head (the group's sentence, the column head with the age axis) repeats on every page it runs onto, and a page breaks only between rows; All packages' heads are plain words, a data date every row shares is said once instead of in a column, and a zero libyears reads "newest"; no section forces a page; a Run data block keeps to its heading; "opens its detail" is not said on paper |
| PD-PRINT-5                  | past page 1 a printed Blast radius row lost its head, key and scale; the Findings axis head read "Reached 0", 3y and 5y touched, and the grey-bar key sat on "Reached"; tails could open a page alone                          | Blast radius's ranked rows print as a table whose head (key, column head, age axis) repeats on every page, rows unbroken, the tails kept to the last row; on paper the Findings age column is 184px with a solid tick at 0 and the grey-bar key between caption and ticks; the packages table has no box                                                                                                                                           |
| PD-ADV-1                    | the tab opened on a count line and a grey group box                                                                                                                                                                            | an answer: advisories, packages, severities, prod/dev, how each is fixed, when reported; "matching" under a filter                                                                                                                                                                                                                                                                                                                                 |
| PD-ADV-2                    | a card per advisory: a pill, a title, one wrapped line of facts                                                                                                                                                                | a ledger row: severity · title, CVE or "no CVE assigned", id · package, way in, prod/dev · range · fix · reported, on one axis                                                                                                                                                                                                                                                                                                                     |
| PD-ADV-3                    | group hints advised ("the cheapest move", "replacement or mitigation"); "Nothing was flagged" when empty                                                                                                                       | groups keep their names and count by severity; empty uses PD-LEDGER-1's two sentences                                                                                                                                                                                                                                                                                                                                                              |
| PD-ADV-4                    | "2 advisories" on Findings rows only, a plain tag                                                                                                                                                                              | one chip on Findings and All packages rows: a square per advisory by severity, the count, fixes in its title                                                                                                                                                                                                                                                                                                                                       |
| PD-ADV-5                    | advisories found by an incomplete check read as the whole list; each row was named by its package alone                                                                                                                        | a "Check incomplete" line under the answer (PD-LEDGER-1's words, Run data link); each row named package, severity, CVE or id                                                                                                                                                                                                                                                                                                                       |
| PD-ADV-6                    | ages under 45 days rounded to "1 mo"; the axis only at the one-line width; group sentences hung under the count                                                                                                                | days, then weeks, under 45 days; a slim axis head over the two-column rows (1024, open package, paper); run-in group heads; phone rows put range and fix together                                                                                                                                                                                                                                                                                  |
| PD-ADV-7                    | the incomplete-check caveat only under the Advisories answer; the band, the tab, the fold and the copied text gave a bare count                                                                                                | one "Check incomplete" tag in band, tab answer and empty state; a ring on the tab count; the fold and the copied line say it                                                                                                                                                                                                                                                                                                                       |
| PD-ADV-8                    | severity and prod/dev counted apart; "years ago" over month ticks; a faint 〃 for a repeated package                                                                                                                           | both scopes counted by severity in the answer; the axis caption names its tick unit; a repeated name stays whole, muted                                                                                                                                                                                                                                                                                                                            |
| PD-BASELINE-1               | a baseline showed only as rail counts and a Run-list sentence                                                                                                                                                                  | a serif delta line (in the summary band since PD-BASELINE-7): new, worsened, accepted as toggles of the Since filter; the entries gone from the lock under it                                                                                                                                                                                                                                                                                      |
| PD-BASELINE-2               | a worsened row said "worsened" alone                                                                                                                                                                                           | "worsened from <previous verdict>"; the tag's title names the baseline file                                                                                                                                                                                                                                                                                                                                                                        |
| PD-BASELINE-3               | the baseline paragraph sat after the checks and follow-the-upstream                                                                                                                                                            | first under the answer, drawn as previous → current pills with lockrot's status word                                                                                                                                                                                                                                                                                                                                                               |
| PD-BASELINE-4               | Run data gave the baseline as one long sentence                                                                                                                                                                                | a stat row (new, worsened, accepted, gone) with the gone names; the Run list keeps "none" only                                                                                                                                                                                                                                                                                                                                                     |
| PD-BASELINE-5               | the gate fact gave no count                                                                                                                                                                                                    | beside it, findings at or above fail-on and, with a baseline, how many it does not accept; never pass or fail                                                                                                                                                                                                                                                                                                                                      |
| PD-BASELINE-6               | a count of findings on the header or on Run data could only be read, not followed                                                                                                                                              | the tally's "N of them not accepted" and Run data's new/worsened/accepted figures each list exactly their findings on Findings (a `focus` action; rail filters verified to return the same set)                                                                                                                                                                                                                                                    |
| PD-BASELINE-7               | the delta line sat over the Findings list, fourth under the band; new/worsened wore the critical/high tones; the rail title broke "LOCKROT-" / "BASELINE.JSON"                                                                 | under the lead, in view on a phone and on paper; new/worsened in the accent on every surface; the rail names the file whole, in mono                                                                                                                                                                                                                                                                                                               |
